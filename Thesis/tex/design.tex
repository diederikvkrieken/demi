			...
			\eIf{j == i}
			{
				...
				\ForEach{$k\in {1,2,...,m}$}
				{
					\eIf{$x^k_{[j,-1]}==\emptyset \parallel u_j(x^k_t) \geq ru_i$}{
						...
						$\Delta_2 u_{jk}(t) \leftarrow u_j(x^k_t)-u_j(x^k_t)-(1-u_j(x^k_{t-1}))$\;
						$\Delta u_jk \leftarrow \max \{\Delta_1u_{jk}(t), \Delta_2u_{jk}(t),0\}$\;
					}
...
\caption{Basic algorithm structure modified from \citet{zheng2015automated}.}
\end{algorithm}
