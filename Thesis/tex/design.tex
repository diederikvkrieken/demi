...
There are two options for an agent to concede. Either the change in utility that the other agents' caused is above the reservation function. In this case, an agent does a concession according to the non-reactive concession strategy as calculated above. 
 ...
 However, if the change is below the reservation function the agents concedes by an amount based on the change the agent perceives in its own utility. This is shown in \Cref{al:algorithm1},  \Cref{al:start} to \Cref{al:end}. \todo[Add pagenumber]{Add pagenumber} \Cpageref{al:algorithm1}
...
\todo[Maken hier meer van?]{Waarom niet $\frac{1}{3}$}
...
\[
\text{Neut utility} = \frac{e^{-A-B}}{e^0}
\] 
